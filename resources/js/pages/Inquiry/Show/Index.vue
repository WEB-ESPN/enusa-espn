<template>
    <Hero title="Inquiry" :category=product.category_code />
        ...
        <Form ref="form" @submit="handleSubmitInquiryForm" :validation-schema="submitInquirySchema" :initial-values="formValues" v-slot="{ errors }">
          <div class="row">
            ...
            <div class="col-md-6">
              ...
              <div>
                <div class="position-relative mt-5">
                  <label style="top: -17px;left: 12px;" class="position-absolute px-2 bg-p-white font-size-16 font-weight-500 " for="product_name">Product Name</label>
...
<script setup>
  ...
  const form = ref(null)
  const productQuery = route.params.code
  ...
  //METHOD
  const getData = () => {
    axios.get(BASE_URL + `frontend/inquiry/product/${productQuery}`)
          .then((RESPONSE) => {
            product.value = RESPONSE.data.data.product
            categories.value = RESPONSE.data.data.categories
            paymentMethods.value = RESPONSE.data.data.paymentMethods
  ...
  const updateBreadcrumb = () => {
    breadcrumb.value = [
        ...
        {
          text: product.value.category_title,
          href: `/products?category=${product.value.category_code}`,
        },
        {
          text: 'Inquiry - ' + product.value.title
        }
      ];
  }
  ...
  const initForm = (data = null) => {
    formValues.value = {
        category: product.value.category_title,
  ...
  const handleSubmitInquiryForm = (values, { resetForm, setErrors }) => {
    ...
    axios.post(BASE_URL + 'frontend/send-product-inquiry', formData)
          .then(res => {
            resetForm()
